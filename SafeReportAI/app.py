...
if page == "About the Model":
    ...
    <div class='metric-card'>
        ...
        label correctly. LR (tuned) correctly labelled 66.24% of all records.<br><br>
        <strong>AUC-ROC (0.7350)</strong> — measures how well the model separates High from Low Priority.
        A score of 0.5 means random guessing. A score of 1.0 means perfect separation.
        0.74 means the model correctly ranks a random High Priority incident above a random
        Low Priority incident 73.50% of the time.<br><br>
        <strong>Recall — High Priority (69.75%)</strong> — of all 7,151 actual High Priority incidents
        in the test set, the model correctly identified 4,988 of them (69.75%).
        It missed 2,163. In a safety context this is the most critical metric —
        a missed urgent incident is more dangerous than a false alarm.<br><br>
        <strong>Precision — High Priority (65.17%)</strong> — of all incidents the model flagged as
        High Priority, 65.17% were actually High Priority. The remaining 34.83% were
        ...
        Tuned LR (C=0.1) was selected with the highest AUC-ROC of 0.7350 — an improvement
        of 1.81 percentage points over the default LR (0.7163). Hyperparameter tuning
        found that C=0.1 (stronger regularisation) generalises better on this dataset.
        RF (tuned) has higher Recall (83.69% vs 69.75%) but lower overall AUC-ROC (0.7157).
        For safety deployments, RF tuned may be preferable due to fewer missed urgent cases.
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("CLASSIFY INCIDENT REPORT"):
        if not any([desc, what_happened, illness, before, obj_substance, location]):
            ...
            if prediction == 1:
                ...
                The model is correct on High Priority cases approximately <b>69.75%</b> of the
                time on unseen data (Recall — tuned LR). A safety officer should always make the final call.
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
