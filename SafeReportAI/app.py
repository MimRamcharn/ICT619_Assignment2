...
<style>
    ...
    .result-table tr:hover td { background: var(--secondary-background-color); }
...
if page == "About the Model":
    ...
    <div class='metric-card'>
        ...
        &nbsp;&nbsp;&nbsp;6 narrative text fields — each processed by its own TF-IDF vectoriser<br><br>
    ...
    with col3:
        st.markdown("**Logistic Regression — Default C=1.0 (Selected)**")
        st.markdown("""
        <table class='result-table'>
            <tr><th>Metric</th><th>Score</th></tr>
            <tr><td>Accuracy</td><td class='val-blue'>64.75%</td></tr>
            <tr><td>AUC-ROC</td><td class='val-blue'>0.7163</td></tr>
            <tr><td>Recall — High Priority</td><td class='val-blue'>65.91%</td></tr>
            <tr><td>Precision — High Priority</td><td class='val-blue'>64.42%</td></tr>
            <tr><td>F1 Score (weighted)</td><td class='val-blue'>64.75%</td></tr>
        </table>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        <table class='result-table'>
            <tr><th>Metric</th><th>Score</th></tr>
            <tr><td>Accuracy</td><td class='val-blue'>63.25%</td></tr>
            <tr><td>AUC-ROC</td><td class='val-blue'>0.7081</td></tr>
            <tr><td>Recall — High Priority</td><td class='val-blue'>85.16%</td></tr>
            <tr><td>Precision — High Priority</td><td class='val-blue'>59.21%</td></tr>
            <tr><td>F1 Score (weighted)</td><td class='val-blue'>61.40%</td></tr>
        </table>
        """, unsafe_allow_html=True)
    ...
    <div class='metric-card'>
        ...
        label correctly. LR correctly labelled 64.75% of all records.<br><br>
        <strong>AUC-ROC (0.7163)</strong> — measures how well the model separates High from Low Priority.
        A score of 0.5 means random guessing. A score of 1.0 means perfect separation.
        0.72 means the model correctly ranks a random High Priority incident above a random
        Low Priority incident 71.63% of the time.<br><br>
        <strong>Recall — High Priority (65.91%)</strong> — of all 7,151 actual High Priority incidents
        in the test set, the model correctly identified 4,714 of them (65.91%).
        It missed 2,437. In a safety context this is the most critical metric —
        a missed urgent incident is more dangerous than a false alarm.<br><br>
        <strong>Precision — High Priority (64.42%)</strong> — of all incidents the model flagged as
        High Priority, 64.42% were actually High Priority. The remaining 35.58% were
        ...
        Default LR (C=1.0) was selected for deployment. Hyperparameter tuning was conducted
        and found that tuned LR (C=0.1) achieved a higher AUC-ROC (0.7350 vs 0.7163), however
        real-world testing showed the tuned model over-relied on vocabulary patterns causing
        false escalations of minor incidents. The default LR provides more balanced and reliable
        classifications in practice.
        RF has higher Recall (85.16% vs 65.91%) meaning it catches more urgent cases but raises
        significantly more false alarms — less suitable for operational deployment.
    </div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col5:
        fig4 = go.Figure()
        fig4.add_trace(go.Bar(x=lr_scores, y=metrics, orientation='h', name='LR (default)', marker_color='#1f77b4', opacity=0.6))
        fig4.add_trace(go.Bar(x=lr_tuned_scores, y=metrics, orientation='h', name='LR (tuned)', marker_color='#1f77b4'))
        layout4 = {**PLOTLY_LAYOUT}
        layout4['xaxis'] = {'range': [0, 1], 'gridcolor': '#2a2f3e', 'tickfont': {'color': '#8892a4'}}
        layout4['height'] = 360
        layout4['barmode'] = 'group'
        fig4.update_layout(**layout4, title='Logistic Regression — Default vs Tuned')
        st.plotly_chart(fig4, use_container_width=True)

    with col6:
        fig5 = go.Figure()
        fig5.add_trace(go.Bar(x=rf_scores, y=metrics, orientation='h', name='RF (default)', marker_color='#ff7f0e', opacity=0.6))
        fig5.add_trace(go.Bar(x=rf_tuned_scores, y=metrics, orientation='h', name='RF (tuned)', marker_color='#ff7f0e'))
        layout5 = {**PLOTLY_LAYOUT}
        layout5['xaxis'] = {'range': [0, 1], 'gridcolor': '#2a2f3e', 'tickfont': {'color': '#8892a4'}}
        layout5['height'] = 360
        layout5['barmode'] = 'group'
        fig5.update_layout(**layout5, title='Random Forest — Default vs Tuned')
